...
definePageMeta({
  // Si tu as un middleware d'auth global (ex: "auth"), remplace la ligne
  // ci-dessous par: middleware: "auth"
});

useHead({ title: "Mon profil — Rift Data" });
  ...
  <template v-else-if="profile">
    ...
    <section class="card">
      ...
      <div v-else class="favorites">
        <NuxtLink v-for="t in profile.favoriteTeams" :key="t.pageid" :to="`/equipes/${t.pageid}`" class="favorite">
          <img v-if="t.logourl" :src="t.logourl" :alt="t.name" class="favorite-logo" />
          <span>{{ t.name }}</span>
        </NuxtLink>
      </div>
    </section>
